...
async function executarDerivarObra() {
  ...
  document.getElementById('btn-derivar-confirmar').disabled = true;
  ...
  try {
    ...
    document.getElementById('derivar-texto').textContent = dados ? 'Encontrado!' : 'Sem resultado, criando obra vazia…';

    const obraId = nome.toLowerCase().replace(/[^a-z0-9]/g, '_');
    let obra = await DB.buscarObra(obraId);
    if (!obra) obra = { id: obraId, titulo: nome, capitulos: [], capaUrl: null, descricao: '' };

    if (dados?.descricao) obra.descricao = dados.descricao;
    if (dados?.capaUrl && !obra.capaUrl) {
      // Tentar baixar capa como dataURL via canvas (pode falhar por CORS em alguns casos)
      try {
        const img = new Image(); img.crossOrigin = 'anonymous';
        await new Promise((res,rej) => { img.onload=res; img.onerror=rej; img.src=dados.capaUrl; });
        const canvas = document.createElement('canvas');
        ...
        obra.capaUrl = canvas.toDataURL('image/jpeg', 0.8);
      } catch { obra.capaUrl = dados.capaUrl; } // fallback: URL direta
    }
    await DB.salvarObra(obra);
