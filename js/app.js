...
async function executarMigracaoAcesso() {
  ...
  try {
    ...
    a.href = url; a.download = `mangavult_backup_${Date.now()}.json`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    URL.revokeObjectURL(url);
    mostrarToast('Backup baixado! ✓ Importe-o em outro navegador via "Remigrar Acesso"');
...
async function executarRemigrarAcesso() {
  ...
  if (!file) { mostrarToast('Selecione o arquivo de backup .json'); return; }
  document.getElementById('btn-remigrar-acesso-confirmar').disabled = true;
  mostrarToast('Importando…', 60000);
  try {
    const text = await file.text();
    const pacote = JSON.parse(text);
    if (!pacote.obras) throw new Error('Arquivo inválido');
    for (const obraData of pacote.obras) {
      ...
      for (const capData of obraData.capitulos) {
        ...
        const blobs = capData.paginas.map(dataUrl => { const arr=dataUrl.split(','); const mime=arr[0].match(/:(.*?);/)[1]; const bstr=atob(arr[1]); const u8=new Uint8Array(bstr.length); for(let i=0;i<bstr.length;i++) u8[i]=bstr.charCodeAt(i); return new Blob([u8],{type:mime}); });
        await DB.salvarPaginasCap(capId, blobs);
    ...
    mostrarToast(`${pacote.obras.length} obra(s) importada(s)! ✓`);
  } catch(err) { mostrarToast('Erro: '+err.message); }
  document.getElementById('btn-remigrar-acesso-confirmar').disabled = false;
}

// ── Migrar capítulo individual via arquivo ──
window.migrarCapitulo = async (obraId, capId) => {
  ...
  try {
    ...
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href=url; a.download=`${obra.titulo}_cap${cap.numero}.mvchapter`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    URL.revokeObjectURL(url);
    mostrarToast(`Cap. ${cap.numero} baixado! Importe via "Remigrar Capítulo"`);
  } catch(err) { mostrarToast('Erro: '+err.message); }
};
...
async function executarRemigrarCap() {
  ...
  mostrarToast('Importando capítulo…', 10000);
  try {
    const text = await file.text();
    const payload = JSON.parse(text);
    if (payload.tipo !== 'cap') throw new Error('Arquivo inválido');
    const capId  = `cap_${Date.now()}`;
    const obraId = payload.obra.id;
    const blobs  = payload.paginas.map(dataUrl => { const arr=dataUrl.split(','); const mime=arr[0].match(/:(.*?);/)[1]; const bstr=atob(arr[1]); const u8=new Uint8Array(bstr.length); for(let i=0;i<bstr.length;i++) u8[i]=bstr.charCodeAt(i); return new Blob([u8],{type:mime}); });
    await DB.salvarPaginasCap(capId, blobs);
    let obra = await DB.buscarObra(obraId);
