...
async function derivarObraWeb(nome) {
  ...
  const data=await r.json(); const item=data?.data?.[0]; if (!item) return null;
  return {
    ...
    tags: [...(item.genres?.map(g=>g.name)||[]),...(item.themes?.map(t=>t.name)||[]),...(item.demographics?.map(d=>d.name)||[])].slice(0,10),
  };
}
